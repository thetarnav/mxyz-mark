...
export default function Movement(): JSX.Element {
    ...
    const position = s.signal(initialPosition),
        isPlayer = s.selector(position),
        playerVec = s.memo(s.map(position, position => matrix.vec(position)))
    ...
    const DirectionKey = (props: { direction: t.Direction }) => {
        ...
        `
        return (
            <div class={clsx(heldDirections.value[props.direction] && 'held')}>
                {props.direction}
            </div>
        )
    }
        ...
        <PlaygroundContainer>
            ...
            <div class="flex items-start gap-16">
                ...
                {untrack(() => {
                    ...
                    const windowed = s.memo(
                        s.map(playerVec, player => t.windowedMatrix(WINDOW_SIZE, player)),
                    )
